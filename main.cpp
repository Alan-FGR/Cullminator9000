		...
		for (int y = 0; y < 1; y++)
			for (int x = 0; x < 10; x++)
			{
				...
				registry.assign<Transform>(entity,
					...
					quat(1, 0, 0, 0),
					vec3((x - 5) * 2, y * 2, (z - 5) * 2));

				registry.assign<Velocity>(entity,
					vec3(0, 0, (std::rand() % 1000) / 100.f) * 0.05f,
					vec3((std::rand() % 1000) / 1000.f, (std::rand() % 1000) / 1000.f, (std::rand() % 1000) / 1000.f) * 0.5f
					);
			}

	//add cullers
// 	auto entity = registry.create();
// 	registry.assign<Transform>(entity, vec3(.1f,.2f,.3f), quat(1, 0, 0, 0), vec3(5));
// 	registry.assign<Velocity>(entity, vec3(0, 0.1f, 0), vec3(0, 1, 1));
	//registry.assign<Frustum>(entity, 50.f, 1.f, 50.f, 2.f);
